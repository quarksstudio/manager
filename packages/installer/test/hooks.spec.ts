...
import { useInstall } from '../src/hooks';
import { install } from '../src/lib/installer';
import { uninstall } from '../src/lib/uninstaller';

jest.mock('../src/lib/installer', () => ({ install: jest.fn() }));
jest.mock('../src/lib/uninstaller', () => ({ uninstall: jest.fn() }));
...
describe('useUninstallPackage', () => {
  it('tracks uninstall progress and completion', async () => {
    ...
    const { useUninstallPackage } = await import('../src/hooks');
