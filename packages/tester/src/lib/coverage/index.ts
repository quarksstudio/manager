...
export class CoverageTracker {
  ...
  schemaPath(path: string): void {
    for (const expected of this.expected.schema)
      if (matchesObligation(expected, path)) this.schema.add(expected);
  }
  assertion(id: string): void {
    this.assertions.add(id);
  }
  invariant(id: string): void {
    this.invariants.add(id);
  }
  report(): CoverageReport {
    const missing = [
      ...this.expected.schema
        .filter((p) => !this.schema.has(p))
        .map((p) => `schema:${p}`),
      ...this.expected.assertions
        .filter((p) => !this.assertions.has(p))
        .map((p) => `assertion:${p}`),
      ...this.expected.invariants
        .filter((p) => !this.invariants.has(p))
        .map((p) => `invariant:${p}`),
    ];
    return {
      schemaCoverage: percent(this.schema.size, this.expected.schema.length),
      assertionCoverage: percent(
        this.assertions.size,
        this.expected.assertions.length,
      ),
      invariantCoverage: percent(
        this.invariants.size,
        this.expected.invariants.length,
      ),
      evaluatedPaths: [
        ...this.schema,
        ...this.assertions,
...
function percent(done: number, total: number): number {
  return total === 0 ? 100 : Number(((done / total) * 100).toFixed(2));
}
...
export function collectSchemaPaths(document: unknown): string[] {
  ...
  const openapiPaths = (
    document as { paths?: Record<string, Record<string, unknown>> }
  ).paths;
  ...
  const properties = (document as { properties?: Record<string, unknown> })
    .properties;
