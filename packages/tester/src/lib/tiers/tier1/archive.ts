...
export async function auditTarArchive(
  ...
  const workspace = await fs.mkdtemp(
    path.join(os.tmpdir(), 'manager-archive-audit-'),
  );
  const archive = path.join(workspace, 'package.tar.gz');
  try {
    if (Buffer.isBuffer(source))
      await fs.writeFile(archive, source, { flag: 'wx' });
      ...
      onentry: (entry) => {
        ...
        if (
          ...
          if (entries.length > MAX_FILES)
            auditError = new Error('Archive contains too many files');
          else if (unpackedBytes > MAX_UNPACKED_BYTES)
            auditError = new Error('Archive is too large when unpacked');
        }
      },
    });
    if (auditError) throw auditError;
    if (!seen.has('skill.yml') && !seen.has('agent.yml'))
      throw new Error(
        'Archive must contain skill.yml or agent.yml at its root',
      );
    return entries.sort();
  } finally {
    await fs.rm(workspace, { recursive: true, force: true });
