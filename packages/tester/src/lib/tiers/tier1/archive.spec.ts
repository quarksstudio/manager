...
describe('archive audit', () => {
  it('accepts a rooted skill archive and rejects traversal', async () => {
    ...
    try {
      ...
      await tar.c({ cwd: root, file: unsafe, gzip: true, prefix: '../' }, ['skill.yml']);
      await expect(auditTarArchive(unsafe)).rejects.toThrow('Unsafe archive path');
  ...
  it('performs the complete Tier 1 archive inspection', async () => {
    ...
    try {
      ...
      await fs.writeFile(path.join(root, 'skill.yml'), [
        'name: demo', 'version: 1.0.0', 'description: Demo package',
        'runtime: python', 'testCommand: python src/tool.py',
        'mapper_files:', '  ".":', '    tools:',
        '      tools/demo.py: src/tool.py', '',
      ].join('\n'));
      const archive = path.join(root, 'demo.tgz');
      await tar.c({ cwd: root, file: archive, gzip: true }, ['skill.yml', 'src/tool.py']);
      const buffer = await fs.readFile(archive);
      const result = await inspectPackageArchive(buffer, { name: 'demo', version: '1.0.0' });
