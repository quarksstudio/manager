...
export async function structuralAudit(
  root: string,
  manifest: PackageManifest,
): Promise<{ hash: string; mappedSources: string[] }> {
  const sources = new Set<string>();
  const mappings = manifest.mapper_files;
  if (!mappings || Object.keys(mappings).length === 0)
    throw new Error('mapper_files must be a non-empty object');
  ...
  if (!sources.size)
    throw new Error('mapper_files must map at least one source file');
  return {
    hash: await canonicalDirectoryHash(root),
    mappedSources: [...sources].sort(),
  };
}
...
async function listFiles(root: string, current = ''): Promise<string[]> {
  ...
  for (const entry of (await fs.readdir(dir, { withFileTypes: true })).sort(
    ...
    if (
      relative === 'skill.lock.yml' ||
      relative === 'agent.lock.yml' ||
      relative === 'outputs' ||
