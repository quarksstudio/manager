...
import { evaluateSkillAssertion } from '../../../../src/lib/tiers/tier2/assertions';

describe('file assertions', () => {
  let root: string;
  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), 'assertion-spec-'));
  ...
  it('validates exact hashes and rejects mismatches', async () => {
    ...
    ).resolves.toBe('outputs/result.bin');
    await expect(
      evaluateSkillAssertion(
        {
          type: 'file_created',
  ...
  it('rejects traversal paths', async () => {
    await expect(
      evaluateSkillAssertion(
        { type: 'file_match', path: '../audit.log', content_regex: 'x' },
        {},
        root,
      ),
    ).rejects.toThrow('Path escapes package root');
  });
});
