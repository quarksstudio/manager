...
for (const group of sourceGroups) {
  ...
  for (const directory of (await readdir(sourceRoot)).sort()) {
    ...
    try {
      ...
      if (!manifest.name || !manifest.version) throw new Error(`${manifestFile} requires name and version`);
      if (packages.has(manifest.name)) throw new Error(`Duplicate package name: ${manifest.name}`);
...
for (const name of order) {
  ...
  const expectedOutput = entry.group === 'apps' ? (entry.manifest.main ?? './index.html') : (entry.manifest.main ?? './index.js');
  if (!(await exists(path.join(output, expectedOutput)))) throw new Error(`Missing build output for ${name}: ${output}/${expectedOutput}`);
  const sourceReadme = path.join(workspace, entry.group, entry.directory, 'README.md');
  if (await exists(sourceReadme)) await cp(sourceReadme, path.join(output, 'README.md'));
...
await writeFile(path.join(distRoot, 'publish-order.json'), `${JSON.stringify(order, null, 2)}\n`);
...
function publishManifest(source, allPackages, group) {
  ...
  if (group === 'packages') {
    ...
    manifest.exports ??= { '.': { types: manifest.types, require: manifest.main, default: manifest.main } };
  } else if (manifest.main) {
    manifest.main = source.main;
  }
  if (manifest.bin) for (const [name, file] of Object.entries(manifest.bin)) manifest.bin[name] = file;
  manifest.files ??= group === 'packages' ? ['**/*.js', '**/*.d.ts', 'README.md'] : ['**/*', 'README.md'];
  ...
  for (const section of ['dependencies', 'optionalDependencies', 'peerDependencies']) {
    for (const [dependency, range] of Object.entries(manifest[section] ?? {})) {
      ...
      if (!local) throw new Error(`${manifest.name} references unknown workspace package ${dependency}`);
...
function topologicalOrder(allPackages) {
  ...
  const visit = (name) => {
    ...
    for (const dependency of Object.keys(dependencies).sort()) if (allPackages.has(dependency)) visit(dependency);
    visiting.delete(name); visited.add(name); result.push(name);
  };
